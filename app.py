...
def admin_re_watermark():
    ...
    # Capturamos solo los IDs: si una foto fue borrada, la recargamos por id
    # dentro del try y la saltamos sin tirar abajo todo el proceso.
    foto_ids = [f.id for f in query.all()]

    ok_count = 0; fail_count = 0; skipped = 0; errores = []

    for fid in foto_ids:
        try:
            foto = db.session.get(Foto, fid)
            if foto is None:
                skipped += 1
                continue
            ...
            db.session.rollback()
            fail_count += 1
            errores.append({'foto_id': fid, 'error': str(e)[:120]})
            print(f'[re-watermark] FAIL foto {fid}: {e}')

    return jsonify({
        'ok': ok_count, 'fallidas': fail_count,
        'saltadas': skipped, 'total': len(foto_ids),
        'errores': errores[:10],
    })
