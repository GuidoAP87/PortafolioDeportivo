...
def agregar_watermark(ruta_entrada, ruta_salida, texto='@NACHO LINGUA'):
    """Marca de agua: 5 filas GIGANTES de @NACHO LINGUA en diagonal, sin sombra,
    sin rejilla. Misma interfaz que antes (archivos en disco, devuelve bool)."""
    try:
        base = Image.open(ruta_entrada).convert('RGBA')

        # Redimensionar preview (RAM de Railway + peso en Cloudinary)
        MAX_PX = 2000
        if max(base.size) > MAX_PX:
            ratio = MAX_PX / max(base.size)
            nuevo = (int(base.size[0] * ratio), int(base.size[1] * ratio))
            base = base.resize(nuevo, Image.LANCZOS)
            print(f'Imagen redimensionada a {nuevo[0]}x{nuevo[1]} para preview')

        overlay = Image.new('RGBA', base.size, (255, 255, 255, 0))
        W, H = base.size

        # Fuente GRANDE: una marca por fila ocupa ~92% del ancho
        fontsize = 10
        font = None
        for path in [
        ...
        # Una marca grande, rotada, repetida en 5 filas
        img = Image.new('RGBA', (tw + 120, th + 120), (255, 255, 255, 0))
        d = ImageDraw.Draw(img)
        d.text((60 - bb[0], 60 - bb[1]), texto, font=font, fill=(255, 255, 255, 150))
        rot = img.rotate(28, expand=True, resample=Image.BICUBIC)
        rc_w, rc_h = rot.size

        px = int(W // 2 - rc_w // 2)
        for frac in [0.12, 0.31, 0.50, 0.69, 0.88]:
            py = int(H * frac - rc_h // 2)
            overlay.paste(rot, (px, py), rot)

        imagen_final = Image.alpha_composite(base, overlay).convert('RGB')

        # Compresion adaptativa (Cloudinary Free rechaza > 10MB)
        ...
        print(f'Preview: {buf.tell()/1024/1024:.1f} MB (quality={quality})')
        return True
    except Exception as e:
        print(f'Error watermark: {e}')
        return False

def agregar_watermark_5x(img_bytes, texto='@NACHO LINGUA', opacidad=150, max_lado=2000):
    """5 filas GIGANTES de @NACHO LINGUA (interfaz de bytes, para subida directa)."""
    base = Image.open(img_bytes).convert('RGBA')
    if max(base.size) > max_lado:
        base.thumbnail((max_lado, max_lado), Image.LANCZOS)
    overlay = Image.new('RGBA', base.size, (255, 255, 255, 0))
    W, H = base.size
    fontsize = 10
    font = None
    ...
    img = Image.new('RGBA', (tw + 120, th + 120), (255, 255, 255, 0))
    d = ImageDraw.Draw(img)
    d.text((60 - bb[0], 60 - bb[1]), texto, font=font, fill=(255, 255, 255, opacidad))
    rot = img.rotate(28, expand=True, resample=Image.BICUBIC)
    rc_w, rc_h = rot.size
    px = int(W // 2 - rc_w // 2)
    for frac in [0.12, 0.31, 0.50, 0.69, 0.88]:
    ...
    buf = io.BytesIO()
    final.save(buf, 'JPEG', quality=85, optimize=True)
    buf.seek(0)
    return buf
